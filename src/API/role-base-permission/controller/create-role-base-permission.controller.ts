...
export const createRoleBasePermissionController = asyncHandler(
  async (req, res) => {
    const { roleId, permissionIds } = req.body as {
      roleId: string;
    ...
    permissionIds.forEach(async (permissionId) => {
      const permission =
        await permissionService.findPermissionById(permissionId);
      if (!permission)
        throw new ApiErrorHandler(
          404,
    ...
    for (const permissionId of permissionIds) {
      const alreadyAssignedPermission =
        await roleBasePermissionService.checkAlreadyAssignedPermission(
          roleId,
          permissionId,
        );
      if (alreadyAssignedPermission)
        throw new ApiErrorHandler(
          400,
          `Permission with ID ${permissionId} already assigned !!`,
        );
    }
    ...
    if (!isAdmin) {
      throw new ApiErrorHandler(
        403,
        "You are not authorized to create role base permission !!",
      );
    }

    // create role base permission
    const roleBasePermission =
      await roleBasePermissionService.createRoleBasePermission(
        roleId,
        permissionIds,
      );
    ...
    return res
      ...
      .json(
        new apiResponse(
          roleBasePermission,
          "Role Base Permission Added successfully !!",
        ),
      );
  },
);
