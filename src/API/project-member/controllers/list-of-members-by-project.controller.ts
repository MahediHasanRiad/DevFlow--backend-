...
export const ListOfMembersByProjectController = asyncHandler(async(req, res) => {

    const projectId = req.params.projectId as string
    if(!projectId) throw new ApiErrorHandler(404, 'Project Id required')

    const projectMemberService = new ProjectMemberService()
    const projectMembers = await projectMemberService.listOfAllMembersByProject(projectId)
